...
def main():
    ...
    page = st.sidebar.selectbox("Select Analysis Type", [
        "Overview & Comparison", "Custom Data Analysis", "Statistical Testing",
        "About"
    ])
...
def show_overview_page():
    ...
    selected_techniques = st.multiselect("Select techniques to compare:",
                                         df['Technique'].tolist(),
    ...
    if len(selected_techniques) >= 2:
        ...
        with col2:
            ...
            for technique in selected_techniques:
                technique_data = filtered_df[filtered_df['Technique'] ==
                                             technique].iloc[0]
                fig_bar.add_trace(
                    go.Bar(name=technique,
                           x=metrics,
                           y=[technique_data[metric] for metric in metrics]))

            fig_bar.update_layout(title="Performance Metrics Comparison",
                                  xaxis_title="Metrics",
        ...
        st.download_button(label="📥 Download Comparison Data (CSV)",
...
def show_manual_entry():
    ...
    with st.form("manual_data_form"):
        ...
        with col1:
            technique_name = st.text_input("Technique Name",
                                           value="Custom Technique")
            true_positives = st.number_input("True Positives (TP)",
                                             min_value=0,
                                             value=85)
            false_negatives = st.number_input("False Negatives (FN)",
                                              min_value=0,
                                              value=15)

        with col2:
            st.write("")  # Spacing
            true_negatives = st.number_input("True Negatives (TN)",
                                             min_value=0,
                                             value=90)
            false_positives = st.number_input("False Positives (FP)",
                                              min_value=0,
                                              value=10)
        ...
        if submitted:
            ...
            results = stats_calc.calculate_diagnostic_stats(
                true_positives, false_negatives, true_negatives,
                false_positives)

            # Display results
            display_custom_results(technique_name, results, true_positives,
...
def show_file_upload():
    ...
    st.markdown(
        "Upload a CSV file with columns: TP, FN, TN, FP, Technique (optional)")
            ...
            if all(col in df.columns for col in required_cols):
                ...
                for idx, row in df.iterrows():
                    technique_name = row.get('Technique',
                                             f'Technique_{int(idx)+1}')
                    results = stats_calc.calculate_diagnostic_stats(
                        int(row['TP']), int(row['FN']), int(row['TN']),
                        int(row['FP']))
...
def display_custom_results(technique_name, results, tp, fn, tn, fp):
    ...
    with col1:
        ...
        confusion_data = [["", "Predicted Positive", "Predicted Negative"],
    ...
    with col2:
        ...
        metrics_df = pd.DataFrame([
            [
                "Sensitivity", f"{results['sensitivity']:.2f}%",
                f"({results['sensitivity_ci'][0]:.2f}% - {results['sensitivity_ci'][1]:.2f}%)"
            ],
            [
                "Specificity", f"{results['specificity']:.2f}%",
                f"({results['specificity_ci'][0]:.2f}% - {results['specificity_ci'][1]:.2f}%)"
            ],
            [
                "PPV", f"{results['ppv']:.2f}%",
                f"({results['ppv_ci'][0]:.2f}% - {results['ppv_ci'][1]:.2f}%)"
            ],
            [
                "NPV", f"{results['npv']:.2f}%",
                f"({results['npv_ci'][0]:.2f}% - {results['npv_ci'][1]:.2f}%)"
            ],
            [
                "Accuracy", f"{results['accuracy']:.2f}%",
                f"({results['accuracy_ci'][0]:.2f}% - {results['accuracy_ci'][1]:.2f}%)"
            ]
        ],
...
def show_statistical_testing_page():
    ...
    col1, col2 = st.columns(2)
    with col1:
        technique1 = st.selectbox("Select first technique:",
                                  df['Technique'].tolist())
    with col2:
        technique2 = st.selectbox("Select second technique:",
                                  df['Technique'].tolist())
    ...
    if technique1 != technique2:
        ...
        kappa_result = stats_calc.calculate_cohens_kappa(
            tech1_data, tech2_data)
        ...
        mcnemar_result = stats_calc.mcnemar_test_simulation(
            tech1_data, tech2_data)
        st.write(
            f"**McNemar's Test p-value:** {mcnemar_result['p_value']:.4f}")
        if mcnemar_result['p_value'] < 0.05:
            st.warning(
                "Significant difference detected between techniques (p < 0.05)"
            )
        else:
            st.success(
                "No significant difference detected between techniques (p ≥ 0.05)"
            )
